...
function deriveFilterOptionsFromData<T>(
  items: T[],
  columns: IColumnDef<T>[],
...
export function OGrid<T>(props: IOGridProps<T>): React.ReactElement {
  ...
  if (!isServerSide && !isClientSide) {
    throw new Error(
      'MaterialDataTable requires either data (client-side) or dataSource (server-side).',
    );
  }
  if (isServerSide && isClientSide) {
  ...
  const clientItemsAndTotal = useMemo(() => {
    ...
    columns.forEach((col) => {
      ...
      if (type === 'multiSelect' && Array.isArray(val) && val.length > 0) {
        ...
        rows = rows.filter((r) =>
          String(getRowValue(r, col.columnId) ?? '')
            .toLowerCase()
            .includes(lower),
        );
        ...
        rows = rows.filter(
          (r) => String(getRowValue(r, col.columnId) ?? '').toLowerCase() === email,
        );
      }
    });
    if (sort.field) {
  ...
  useEffect(() => {
    ...
    dataSource
      ...
      .then((res: { items: T[]; totalCount: number }) => {
  ...
  const displayItems =
    isClientSide && clientItemsAndTotal ? clientItemsAndTotal.items : serverItems;
  const displayTotalCount =
    isClientSide && clientItemsAndTotal ? clientItemsAndTotal.totalCount : serverTotalCount;
  ...
  return (
    <Box className={className} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        {title != null ? <Box sx={{ m: 0 }}>{title}</Box> : null}
        {toolbar}
        <ColumnChooser
